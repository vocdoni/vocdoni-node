...
// made them collide, so both derivations stay documented in one place.
...
// TestSaltPubKeyLegacyTruncation pins the legacy behaviour: the salting
// primitives consume only the first SaltSize bytes, so passing a full 32 byte
// processID is identical to passing its first 20. OFF_CHAIN_CA elections rely
// on this staying exactly as it is.
