...
func EthEvents(ethProcDomain, w3host string, w3port int, startBlock int64, endBlock int64, subscribeOnly bool,
	cm *census.Manager, signer *signature.SignKeys, evh []ethevents.EventHandler) error {
	...
	ev, err := ethevents.NewEthEvents(contractAddr, signer, fmt.Sprintf("ws://%s:%d", w3host, w3port), cm)
	if err != nil {
