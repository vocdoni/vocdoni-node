...
func (app *BaseApplication) Info(_ context.Context,
	...
	if lastHeight > 0 && lastHeight < uint32(app.Genesis().InitialHeight) {
		...
		if !bytes.Equal(appHash, app.Genesis().AppHash) {
			log.Fatalf("our current appHash (%x) doesn't match the genesis initial AppHash (%x), aborting...")
		}
		lastHeight = 0
	}
...
func (app *BaseApplication) ProcessProposal(_ context.Context,
	...
	if config.ForksForChainID(app.ChainID()).EndOfChain > 0 &&
		app.State.CurrentHeight() >= config.ForksForChainID(app.ChainID()).EndOfChain {
		log.Errorf("reached end of chain %s at height %d, rejecting new blocks", app.ChainID(), app.State.CurrentHeight())
		log.Errorf("now you can deploy a new chain on top, using a genesis with a different ChainID, InitialHeight=%d and AppHash=%x",
