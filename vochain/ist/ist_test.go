...
func TestISTCschedule(t *testing.T) {
	...
	// start block 2, on this block results should be computed
	testAdvanceBlock(t, s, istc)

	// start block 3, on this block results should be commited
	testAdvanceBlock(t, s, istc)
...
func TestISTCsyncing(t *testing.T) {
	...
	err = istc.Commit(s.CurrentHeight(), true)
	qt.Assert(t, err, qt.IsNil)
	_, err = s.Save()
	qt.Assert(t, err, qt.IsNil)
